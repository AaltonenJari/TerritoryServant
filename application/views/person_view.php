    ...
    <div id="filterArea" class="filterArea">
      <table id="selectortable">
        <tr>
          ...
          <td>
		    <input type="hidden" id="selGroupLimitOld" value="<?php echo $group_limit_sel; ?>" />
      		<select name="groupLimitChkBoxChooser" id="groupLimitChkBoxChooser" onChange="jsFunction4()">
         	  <option value="0" <?php if ($group_limit_sel == "0") echo "selected=\"selected\""?> >Kaikki</option>
			  <?php 
			  foreach ($group_limit_codes as $key=>$value) { ?>
			      <option value="<?php echo $value; ?>" <?php if ($group_limit_sel == $value) echo "selected=\"selected\""?> ><?php echo $key; ?></option>
              <?php }
              ?>
      	    </select> 
          </td>
              ...
              <tr>
                  <?php foreach ($display_fields as $field_name => $field_display) { ?>
                      <th <?php if ($sort_by == $field_name) echo "class=\"sort_" . $sort_order . "\"" ?>><span>
                          ...
                          <?php if (!empty($group_limit_sel)) { $hdrurl .= "/" . $group_limit_sel; } else { $hdrurl .= "/0"; } ?>
                       ...
                       <?php foreach ($display_fields as $field_name => $field_display) {
                           ...
                           switch ($field_name) {
                               ...
                               default: ?>
                                   ...
                                   $js = ['onChange' => 'jsFunction2(' . $field_input_name_id . ', ' . $personid . ');'];
    ...
    <div id="bottomArea" class="bottomArea">
      <table id="bottomtable">
        <tr>
          ...
          <td width="15%">
            <div id="reportPrint">
              <input type="button" value="Raportti" class="btnAction" onclick="personMaintenanceToPDF()" />
            </div>
          </td>
        </tr>
      </table>
...
$('#filterString').keyup(function() {
  ...
  document.getElementById("tunnus").href = document.getElementById("tunnusold").value +
        "\\" + document.getElementById("selGroupLimitOld").value +
...
function jsFunction4() {
	var myselect = document.getElementById("groupLimitChkBoxChooser");
	document.getElementById("selGroupLimitOld").value = myselect.options[myselect.selectedIndex].value;
